...
#         form = forms.LoginForm(initial={"email": "csk@naver.com"})
#         return render(request, "users/login.html", {"form": form})
...
def log_out(request):
    messages.info(request, "See you lager")
    logout(request)
    return redirect(reverse("core:home"))
...
def github_login(request):
    ...
    redircet_uri = "http://127.0.0.1:8000/users/login/github/callback"
    return redirect(
        f"https://github.com/login/oauth/authorize?client_id={client_id}&redircet_uri={redircet_uri}&scope=read:user",
    )
...
def kakao_callback(request):
    try:
        ...
        if email is None:
            raise KakaoException("Please also give me your eamil")
